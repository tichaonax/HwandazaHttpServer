...
export class HwandaLoggedInContainer extends React.Component {
   /*  componentDidMount() {
        const {dispatch, currentURL, isLoggedIn, router} = this.props
        const redirectUrl = currentURL.pathname + currentURL.search;
        //console.debug("HwandaLoggedInContainer redirectUrl", redirectUrl);
        if (!isLoggedIn) {
...
export default connect()(HwandaLoggedInContainer)